...
/// struct for typed errors of method [`webhook_dispatch_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum WebhookDispatchGetError {
...
/// struct for typed errors of method [`webhook_dispatches_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum WebhookDispatchesGetError {
    Status400(models::ErrorResponse),
    Status401(models::ErrorResponse),
    Status403(models::ErrorResponse),
    Status405(models::ErrorResponse),
...
pub async fn webhook_dispatch_get(configuration: &configuration::Configuration, dispatch_id: &str) -> Result<models::WebhookDispatchResponse, Error<WebhookDispatchGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_dispatch_id = dispatch_id;

    let uri_str = format!("{}/v2/webhook-dispatches/{dispatchId}", configuration.base_path, dispatchId=crate::apis::urlencode(p_path_dispatch_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::WebhookDispatchResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::WebhookDispatchResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<WebhookDispatchGetError> = serde_json::from_str(&content).ok();
...
pub async fn webhook_dispatches_get(configuration: &configuration::Configuration, offset: Option<f64>, limit: Option<f64>, desc: Option<bool>) -> Result<models::ListOfWebhookDispatchesResponse, Error<WebhookDispatchesGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_offset = offset;
    let p_query_limit = limit;
    let p_query_desc = desc;

    let uri_str = format!("{}/v2/webhook-dispatches", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<WebhookDispatchesGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
