...
/// struct for typed errors of method [`users_me_limits_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UsersMeLimitsGetError {
    Status400(models::ErrorResponse),
    Status401(models::ErrorResponse),
    Status403(models::ErrorResponse),
    Status405(models::ErrorResponse),
...
/// struct for typed errors of method [`users_me_limits_put`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UsersMeLimitsPutError {
    Status400(models::ErrorResponse),
    Status401(models::ErrorResponse),
    Status403(models::ErrorResponse),
    Status405(models::ErrorResponse),
...
/// struct for typed errors of method [`users_me_usage_monthly_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UsersMeUsageMonthlyGetError {
    Status400(models::ErrorResponse),
    Status401(models::ErrorResponse),
    Status403(models::ErrorResponse),
    Status405(models::ErrorResponse),
...
pub async fn user_get(configuration: &configuration::Configuration, user_id: &str) -> Result<models::PublicUserDataResponse, Error<UserGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_user_id = user_id;

    let uri_str = format!("{}/v2/users/{userId}", configuration.base_path, userId=crate::apis::urlencode(p_path_user_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PublicUserDataResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PublicUserDataResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn users_me_get(configuration: &configuration::Configuration, ) -> Result<models::PrivateUserDataResponse, Error<UsersMeGetError>> {

    let uri_str = format!("{}/v2/users/me", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PrivateUserDataResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PrivateUserDataResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn users_me_limits_get(configuration: &configuration::Configuration, ) -> Result<models::LimitsResponse, Error<UsersMeLimitsGetError>> {

    let uri_str = format!("{}/v2/users/me/limits", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::LimitsResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::LimitsResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<UsersMeLimitsGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn users_me_limits_put(configuration: &configuration::Configuration, update_limits_request: Option<models::UpdateLimitsRequest>) -> Result<serde_json::Value, Error<UsersMeLimitsPutError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_update_limits_request = update_limits_request;
    ...
    req_builder = req_builder.json(&p_body_update_limits_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<UsersMeLimitsPutError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn users_me_usage_monthly_get(configuration: &configuration::Configuration, date: Option<&str>) -> Result<models::MonthlyUsageResponse, Error<UsersMeUsageMonthlyGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_date = date;

    let uri_str = format!("{}/v2/users/me/usage/monthly", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::MonthlyUsageResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::MonthlyUsageResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<UsersMeUsageMonthlyGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
