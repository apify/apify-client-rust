...
| `get_streamed_log()` | — | `Result<impl Stream<Item = Result<Vec<u8>>>>` | Convenience for `log().stream()` — streams the run's log chunks live (log redirection); `async`, so `.await?` the returned stream. |
| `get_streamed_log_with_options(options)` | `LogOptions` | `Result<impl Stream<Item = Result<Vec<u8>>>>` | As `get_streamed_log()`, forwarding `LogOptions` (e.g. `raw`) to the log stream. |
